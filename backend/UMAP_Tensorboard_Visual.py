...
!pip install fair-esm tensorboard umap-learn
...
writer.add_embedding(X_combined, metadata=metadata_tb, metadata_header=["Type", "Label"])

writer.close()
#print("Done logging!")


#-------Launch TensorBoard inside Colab-------#
...
#Convert to numpy bec UMAP expects that
X_np = X.numpy()

#Fit UMAP on my embeddings
...
for i, label in enumerate(metadata):
    plt.text(X_umap[i, 0], X_umap[i, 1], label[:10], fontsize=6)
    #print(label)
